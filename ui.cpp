...
int disconnect()
{
	...
	if(ERROR_SUCCESS == RasHangUp(hRasConn))
	{
    	...
    	RasDeleteEntry(NULL, L"Alto Grade IPSEC IKE VPN");
...
// int start()
{
	...
	if (ERROR_SUCCESS == dwRet)
	{
		for (DWORD i = 0; i < dwDevices; i++)
		{
			...
			if(NULL != wcsstr(lpRasDevInfo[i].szDeviceName, L"IKEv2"))
			{
				wprintf(L"%s: ", lpRasDevInfo[i].szDeviceName);
				wprintf(L"%s\n", lpRasDevInfo[i].szDeviceType);
	...
	printf("indexOfIPSECIKEDevice: %d\n", indexOfIPSECIKEDevice);
    ...
    pRasEntry->dwFramingProtocol = RASFP_Ppp;  // ??????????????????
 	...
 	hr = StringCchCopy(pRasEntry->szLocalPhoneNumber, ARRAYSIZE(pRasEntry->szLocalPhoneNumber), L"192.168.1.3");
	if(FAILED(hr))
	{
		printf("FAILED hr 1\n");
	}

	hr = StringCchCopy(pRasEntry->szDeviceName, ARRAYSIZE(pRasEntry->szDeviceName), lpRasDevInfo[indexOfIPSECIKEDevice].szDeviceName);
	if(FAILED(hr))
	{
		printf("FAILED hr 2\n");
	}

	hr = StringCchCopy(pRasEntry->szDeviceType, ARRAYSIZE(pRasEntry->szDeviceType), lpRasDevInfo[indexOfIPSECIKEDevice].szDeviceType);
	if(FAILED(hr))
	{
		printf("FAILED hr 3\n");
	}
	...
	StringCchCopy(lpRasDialParams->szEntryName, ARRAYSIZE(lpRasDialParams->szEntryName), L"Alto Grade IPSEC IKE VPN");
	if(FAILED(hr))
	{
	...
	switch (uMsg)
	{
		...
		case WM_CREATE:
		{
				  ...
				  /***************** EXPERIMENT *********************************/
						...
						ShellExecute(NULL,  L"runas",  L"\"installCertificates.exe\"", NULL, NULL, 5);
		...
		case WM_PAINT:
		{
			   ...
			   // StringCchCopy( lf->lfFaceName, LF_FACESIZE, L"Arial"); 
				...
				HFONT hFont = CreateFontIndirect(lf);
				HDC hdcc = BeginPaint(btn, &ps);	
				SelectObject(hdcc, hFont);
				SendMessage(btn, WM_SETFONT, (WPARAM)hFont,  MAKELPARAM(TRUE, 0));	 */
...
PWSTR lpCmdLine, INT nCmdShow)
{
	...
	HWND hwnd = CreateWindowEx(
		0, // Optional window styles.
		 ...
		 L"Alto Grade IPSEC-IKE VPN client", // Window text
